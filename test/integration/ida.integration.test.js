   ...
   afterEach(async () => {
    ...
    it("Get critical after IDA distribuiton", async () => {
        try {
            ...
            await protocolVars.host.methods.callAgreement(protocolVars.ida._address, data, "0x").send({from: accounts[0], gas: 1000000});
            const subscriptionData = protocolVars.ida.methods.updateSubscription(
                protocolVars.superToken._address, 6, accounts[1], 100, "0x"
            ).encodeABI();
            await protocolVars.host.methods.callAgreement(protocolVars.ida._address, subscriptionData, "0x").send({from: accounts[0], gas: 1000000});
            const approveSubData = protocolVars.ida.methods.approveSubscription(
                protocolVars.superToken._address,accounts[0],6,"0x"
                ).encodeABI();
            await protocolVars.host.methods.callAgreement(protocolVars.ida._address, approveSubData, "0x").send({from: accounts[1], gas: 1000000});
            await ganache.helper.timeTravelOnce(60);

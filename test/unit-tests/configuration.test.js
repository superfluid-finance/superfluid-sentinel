...
const populateEnvVariables = () => {
  ...
  process.env.NO_REMOTE_MANIFEST = false;
  return {
    ...
    NO_REMOTE_MANIFEST: process.env.NO_REMOTE_MANIFEST
  };
};
...
const removeEnvVariables = () => {
  ...
  delete process.env.NO_REMOTE_MANIFEST;
};
...
describe("Test Agent user configurations", () => {
  it("Boot agent with env variables", async () => {
    try {
      ...
      expect(envObj.NO_REMOTE_MANIFEST.toString()).to.equal(config.NO_REMOTE_MANIFEST.toString());
      removeEnvVariables();
    } catch (err) {
      exitWithError(err);
    }
